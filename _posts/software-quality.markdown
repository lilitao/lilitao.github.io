...
## 业务建模与分析
在项目开始，BA完成分析并确定需求后，开发人员（包括SA、一般开发人员)开始入驻编写软件代码，我们SA首先遇到的问题是抽象实际的业务,SA通过需求说明文档或与BA沟通，甚至与user沟通来理解学习业务知识,然后抽象我们遇到的实际业务，抽象的结果就是使用软件的方式去对实际业务建模，产出的结果是建模文档（系统功能详细设计或者javaDoc）。在这里我们介绍使用DDD的思想去分析抽象业务，使用`plantUml`这个工具帮我理清思路.

1. 确实我们软件项目中的代码分类，我们把们所有的业务代码都归类到以下几种类型：
...
* application service:
* domain model : 处理核心问题领域业务，基于DDD的核心思想
* domain service : 业务处理器，处理domain model之间的交互,返回业务处理结果给调用者
* domain repository : 处理核心技业务的持久化
* domain factory(builder):负责创建工作
* domain event : 业务事件
* domain listener: 监听业务事件，实现业务逻辑，解耦复杂的业务逻辑
* data access persistence object : 持续久化对象


2. 使用DDD的思想把业务中的关键名词、动词找出来，并归类出业务中的Entity和命令动作(DDD的实践与应用请参考DDD相关的课程文档),在这个过程中使用plantUml来理清思路
